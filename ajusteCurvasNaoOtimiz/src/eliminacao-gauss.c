...
int find_max(intervaloDouble **A, int col, int n) {
    int max_row = col;
...
// troca duas linhas
void swap_line(intervaloDouble **A, intervaloDouble *b, int row1, int row2, int n) {
...
void gauss_elim(intervaloDouble **A, intervaloDouble *b, int n) {
    for (int i = 0; i < n; i++) {
...
void retrossubs(intervaloDouble **A, intervaloDouble *b, intervaloDouble *x, int n) {
    for(int i = n-1; i >= 0; i--) {
        x[i].max = b[i].max;
        x[i].min = b[i].min;
        for(int j = i+1; j < n; j++)
